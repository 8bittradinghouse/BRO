...
| `order_rate_recovery_reserved_slots` + `reduce_only_recovery_active` family | removed recovery-rate / recovery-authority lineage with ignored dead-key compatibility only | may explain packet-era ancestry and artifact rereads | may not backfill maker permission, reintroduce maker recovery relaxations, or impersonate a live safety spine | `CUT / HISTORICAL-COMPAT LINEAGE` |
