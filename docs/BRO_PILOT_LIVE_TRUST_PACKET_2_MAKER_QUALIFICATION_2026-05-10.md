...
- prior packet closure dependency:
  - `docs/BRO_PILOT_LIVE_TRUST_PACKET_1_TAKER_QUALIFICATION_2026-05-06.md`
- governing question:
  - is current maker economically safe enough to arm, or do mixed authority,
    support-tool drift, historical residue, and unresolved economic wounds still
...
7. consumer and support surfaces:
...
8. structural atlas and historical forensics as reference only:
...
Mission lock:
- prove maker is economically safe enough to arm in principle, or prove that it
  is not
...
- runtime truth outranks wrapper summaries

Authority lock:
- current board owners outrank older maker packet wording
- runtime emitters outrank shop-tool compression
- blueprint intent informs the packet; it does not bypass doctrine/runtime truth

Pathology lock:
- likely disease is mixed authority plus unresolved economic wound geometry, not
  merely “maker is inactive”

Semantic lock:
- support tools may assist diagnosis but may not impersonate runtime owner law
- drift-era maker timing language is historical-only
- history stays history unless current code re-proves it

Timing lock:
- host -> runtime -> decision -> submit -> report remains one combat timing
  chain

Economic-meaning lock:
- truth populations must stay separated:
  - decision-cycle truth
  - submit truth
  - filled-order truth
  - fill-event truth
  - complete-outcome truth
- small recurring negative maker losses must be traced to an exact truth
  population before any response is legal

False-closure list:
- “maker got fills” as proof that maker is safe to arm
- “good paper wrapper” as proof that maker economics are clean
- “historical timing packet passed” as current doctrine proof
- “FMA says so” as runtime-owner truth
- “bad participation means tune it” before owner cleanup is complete
...
- one seam or one tightly coupled authority set per slice
- contradiction compression before closure
- negative-proof before closure
- preserve explicit residuals instead of smoothing them away
...
- `FMA` is a shop compression/instrumentation layer, not BRO core doctrine or
  runtime authority
- Packet 2 must decide what stays as a fenced support tool, what becomes
  explicit runtime steel, and what gets cut

## Historical Closure Quarantine
Allowed to answer:
- what drift-era maker packets were trying to protect
- what runtime posture existed at the time
- which wound families were historically real

Explicitly forbidden to answer:
- whether current maker is live-trustworthy
- whether drift-era timing language is current doctrine
- whether support-tool output outranks current runtime owner truth
