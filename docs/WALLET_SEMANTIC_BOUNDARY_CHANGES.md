...
| tx-manager `pending_tx_snapshot` consumed as live nonce/pending truth | `local_tx_lifecycle_state.exchange_intent_snapshot` | `local` |
...
| legacy flat status compatibility fields on authority path | retired; authoritative decisions consume canonical/derived domain surfaces only | `retired` |
...
- `local_tx_lifecycle_state.exchange_intent_snapshot` is the truthful local
  name for CLOB submit/open/cancel lifecycle context.
- `local_tx_lifecycle_state.pending_tx_snapshot` has been retired. Local
  exchange-plane truth lives only at
  `local_tx_lifecycle_state.exchange_intent_snapshot`.
...
- Deprecated legacy surfaces have been retired from the live authority status
  contract.
- Authoritative decisions consume canonical domain surfaces, not deprecated
  aliases.
