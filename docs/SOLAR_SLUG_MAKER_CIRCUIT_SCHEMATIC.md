...
- the new cannon-shadow overlays now also tell us whether the sampled fights are
  even inside the intended `15-20s` doctrine box, whether the book has `1.5x`
  depth versus the `$350` doctrine shot, and whether secondary-oracle
...
- current answer:
  - transition and asia samples still fail on `insufficient_depth_multiple`
    and `secondary_oracle_unknown` before reaching any latent full-candidate
    state
