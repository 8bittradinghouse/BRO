#!/usr/bin/env python3
"""Backstage qualification gate for BRO paper harness hardening.

Canonical paper harness proof remains primary. Auxiliary hardening checks must
not compete with canonical paper truth.
"""
...
def run_gate(
    *,
    ...
    fault_configured = bool(drill_policy.get("enabled", False))
    ...
    elif fault_configured and bool(force_skip_fault_drill):
        checks["network_fault_drill_audit"] = {"skipped": True, "reason": "operator_skip"}
        decision_trace.append(
            decision_item(
                check="network_fault_drill_audit",
                level="advisory",
    ...
    reported_policy: Dict[str, Any] = {"paper_harness": paper_policy}
    if fault_configured:
        reported_policy["fault_drill"] = drill_policy
...
def main() -> None:
    parser = argparse.ArgumentParser(description="BRO backstage paper-harness qualification gate")
